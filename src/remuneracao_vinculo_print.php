...
<?php
    ...
    $tabela = file_get_contents("../downloads/Renumeracao_Vinculo_{$nr_ano}{$nr_mes}_{$md5_unidade}.html");
?>
<html>
    <head>
        <?php
...
//
                ...
                if (intval($id_crg) !== 0) {
                    $sql = 
                        ...
                        ."where c.id_cargo = " . intval($id_crg);

                    $res = $pdo->query($sql);
                    if (($obj = $res->fetch(PDO::FETCH_OBJ)) !== false) {
                        $ds_crg = $obj->descricao;
                    }
                }
            } catch (Exception $ex) {
        ...
        <?php
            if (intval($id_crg) !== 0) {
                echo "<p>Cargo/Função : <strong>{$ds_crg}</strong></p>";
            }
        ?>
        <br>
        <?php
