...
abstract class Dao {
    ...
    public abstract function getFieldValueDB($table_name, $field_name, $where);
}
...
class DaoFireBird extends Dao {
    public function getCountID($table_name, $field_name) {
        try {
            ...
            $sql = "Select (Select coalesce(max({$field_name}), 0) + 1  from {$table_name}) ID from VW_INFORMACOES"; 
            $res = $pdo->query($sql);
    ...
    public function getFieldValueDB($table_name, $field_name, $where) {
        try {
            $cnf = Configuracao::getInstancia();
            $pdo = $cnf->db('', '');

            $sql = "Select (Select {$field_name}  from {$table_name} where {$where}) VALOR from VW_INFORMACOES"; 
            $res = $pdo->query($sql);
            ...
            return "";
            echo $ex . "<br><br><strong>Error Dao::getInstancia->getFieldValueDB() : </strong>" . $ex->getMessage();
        } 
    }
}