...
function configurarTabelaEvento(){
    // Configurando Tabela
...
function consultarEvento(id, us) {
    ...
    if (parseInt(params.to) === 0) {
        mensagem_alerta("Selecione o cliente para que sua Tabela de Eventos seja carregada");
    } else {
        // Iniciamos o Ajax 
        $.ajax({
            // Definimos a url
...
function editarEvento(id) {
    var id_evento = id.replace("editar_evento_", "");
    
    $('#op').val("editar_evento");
    
    $('#id_evento').val( id_evento );
    $('#id_cliente_cadastro').val( $('#id_cliente_' + id_evento).val() );
    $('#codigo').val( $('#codigo_' + id_evento).val() );
    $('#descricao').val( $('#descricao_' + id_evento).val() );
    $('#tipo').val( $('#tipo_' + id_evento).val() );
    $('#tipo_lancamento').val( $('#tipo_lancamento_' + id_evento).val() );
    $('#sem_uso').prop('checked', ($('#sem_uso_' + id_evento).val() === 'S')); 

    $('#id_cliente_cadastro').trigger('chosen:updated');
    $('#tipo').trigger('chosen:updated');
    $('#tipo_lancamento').trigger('chosen:updated');
