...
function configurarTabelaUnidadeGestora(){
    // Configurando Tabela
    var table = $('#datatable-responsive').DataTable({
        "paging": true,
        "pageLength": 5, // Quantidade de registros na paginação
        "lengthChange": false,
        "searching": true,
        "ordering": true,
        "info": true,
        ...
        "columnDefs": [
            ...
            {"orderable": false, "targets": 5}  // Controles
        ],
        "order": [[2, 'asc']], // <-- Ordenação 
        "language": {
                "paginate": {
...
function consultarUnidadeGestora(id, us) {
    ...
    if (parseInt(params.to) === 0) {
        mensagem_alerta("Selecione o cliente para que sua Tabela de Unidades Gestoras seja carregada");
    } else {
        // Iniciamos o Ajax 
        $.ajax({
            // Definimos a url
            url : './unidade_gestora_dao.php',
            ...
            success : function(data){
                $('#page-wait').html("");
                $('#tabela-unidades_gestoras').html(data);

                $('#btn_consultar').attr('disabled', false);
                configurarTabelaUnidadeGestora();
            },
            error: function (request, status, error) {
                $('#page-wait').html("");
                $('#tabela-unidades_gestoras').html("Erro na execução da pesquisa!<br> (" + status + ")" + request.responseText + "<br><strong>Error : </strong>" + error.toString());
...
function editarUnidadeGestora(id) {
    var id_unidade = id.replace("editar_unidade_", "");
    
    $('#op').val("editar_unidade");
    
    $('#id').val( $('#id_' + id_unidade).val() );
    $('#id_cliente_cadastro').val( $('#id_cliente_' + id_unidade).val() );
    $('#cod_tcm').val( $('#cod_tcm_' + id_unidade).val() );
    $('#cnpj').val( $('#cnpj_' + id_unidade).val() );
    $('#razao_social').val( $('#razao_social_' + id_unidade).val() );
...
function excluirUnidadeGestora(id) {
    var id_unidade = id.replace("excluir_unidade_", "");
    var i_linha = document.getElementById("linha_" + id_unidade);
    mensagem_confirmar("Confirma a exclusão da Unidade Gestora <strong>'" + $('#razao_social_' + id_unidade).val() + " (" + $('#cnpj_' + id_unidade).val() + ")'</strong>?", function(){
            ...
            $.ajax({
                ...
                success : function(data){
                    ...
                    if (retorno === "OK") {
                        ...
                        $('#btnF_confirma_msg').trigger("click");
                        mensagem_erro( "<p><strong>Erro ao tentar excluir a Unidade Gestora selecionada:</strong> <br><br>" + retorno + "</p>" );
                    }
                },
                error: function (request, status, error) {
                    $('#btnF_confirma_msg').trigger("click");
